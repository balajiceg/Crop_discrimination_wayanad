...
var all_bands=fused_vv.clip(sulthanb);
...
// print(ee.FeatureCollection(testingPartition).aggregate_histogram('LULC'))
  ...
  for(var n_tree=10;n_tree<=150;n_tree=n_tree+2){
    // classifier defining
    var classifier = ee.Classifier.randomForest({numberOfTrees:n_tree});
    ...
    print("<<<<< n_tree= "+n_tree+" >>>>>",confusionMatrix.accuracy());
    // print("<<<<<<<<<<--->>>>>>>>>")
  }
